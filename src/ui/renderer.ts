...
export function renderSessionInfo(session: SessionSummary): string {
  ...
  if (session.lastTimestamp && session.lastTimestamp !== session.startTimestamp) {
    lines.push(`Finished:   ${session.lastTimestamp.toLocaleString()}`);
    const duration = Math.floor((session.lastTimestamp.getTime() - session.startTimestamp.getTime()) / 1000);
...
export function renderSessionMessage(
  ...
  const colorFn = isToolUse ? colors.gray : (type === 'user' ? colors.cyan : colors.white);
